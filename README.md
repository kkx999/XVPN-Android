...
`v1.0.0`（`versionCode 10020`）

1.0.0 基于已确认可连接的 RC2 收口发布，Application ID、固定签名链和 sing-box/libbox 1.13.19 数据面保持不变。源码包含真实联网健康检查、热切换回滚、连接态真实节点优选、内置 Panel 地址托管迁移与正式版交互细节；正式 APK 仍须完成 `QA_CHECKLIST.md` 的真机验收。
...
- 默认 Panel 使用 `https://xvpn.666101.xyz`。1.0.0 以此作为新的内置地址基线；后续更换内置地址时，仍使用内置地址的用户会自动迁移，手动保存的自定义 Panel 地址保持不变。
