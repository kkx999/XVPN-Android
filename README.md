...
`v1.1.1-rc1`（`versionCode 10101`）

1.1.1-rc1 在 1.1.0 基线上补齐 Always-on、强制最低版本、应用内安全更新、IPv4/IPv6 防泄漏、节点抽屉手势和面板流量实时性；Application ID、固定签名链和 sing-box/libbox 1.13.19 数据面保持不变。发布正式 APK 前仍须完成 `QA_CHECKLIST.md` 的真机协议验收。
...
- TUN 同时接管 IPv4 与 IPv6 默认路由，使用 MTU 1400、`mixed` 栈、`strict_route` 与 DNS 劫持。国内域名和节点域名由直连 `223.5.5.5` UDP 负责，其他域名使用经代理隧道传输的 `8.8.8.8 TCP DNS`；DNS 仍优先返回 IPv4，避免本地 IPv6 绕过隧道。
...
- 已连接时可以切换智能分流 / 全局代理、打开节点列表、手动换节点或自动优选。首次连接与热重载不仅执行 libbox `checkConfig`，还会通过当前 TUN 请求保存的 HTTP/HTTPS 联网检测网站；新配置无法解析 DNS 或访问代理出口时自动回滚原节点与原模式，不会发布虚假“已连接”。
- 节点列表、节点详情与首页统一显示物理网络上的节点入口 TCP 握手延迟，计时不包含 DNS 查询；连接前后使用同一探测方法，绝不再经当前隧道回退生成 1–5 ms 的假结果。
- 已连接自动优选不会逐个断开重连，而是在保持当前连接时并发比较入口延迟，只对最终最快节点执行一次热切换和真实联网验证；首次联网检测仅在第一次失败后自动重试一次。
- 用户保存的联网检测网站用于首次连接、模式切换、手动换节点、自动优选最终验证及连接诊断；旧版 gstatic 内置值自动迁移到 Apple `success.html`。
- 首页连接成功后显示当前出口 IP、运营商以及机房 / 家宽和地区匹配的智能识别；识别结论采用保守标签，不作为线路属性承诺。
- admin 账户不再被客户端关闭流量上报；同时维护本机日 / 月 / 累计账本，在 Panel 暂未返回统计字段时仍能显示实际使用量。
- 节点底部面板改为与系统导航手势区同色衔接，浅色与深色模式不再出现突兀的独立底条。
...
- 右上角刷新改为随主题变化的圆形冰晶按钮与对称双箭头，刷新时旋转并在完成时给出克制回弹反馈。
...
- 支持 Android 系统 Always-on 与“屏蔽未使用 VPN 的连接”；仅在真实隧道验证成功后，用 Android Keystore 加密保存恢复所需的最小配置，注销、撤销授权或账户/节点失效时主动清除。
- Panel 最低版本限制在 API 层以 HTTP 426 强制执行；更新元数据完整时，App 内完成 HTTPS 下载、大小/SHA-256、包名、版本和固定签名校验后再调用系统安装器。

## 当前测试边界

- CPU：1.1.1-rc1 仅包含 `arm64-v8a`，用于当前 arm64 Android 16 真机及同架构设备。
- 系统：Android 8.0+（minSdk 26），target / compileSdk 36。
- 流量：`TrafficStats` 是 App/Core UID 的代理侧统计，适合产品展示与 Panel 汇总，不是防篡改计费依据。
- 仍需真机验证各节点协议、Wi-Fi / 蜂窝切换、长时间锁屏、分流覆盖、DNS 检测以及各厂商通知栏样式。正式 Release 应保存完整验收记录。
...
4. 下载 Artifact `XVPN-v1.1.1-rc1`，其中包含 APK 与 `SHA256SUMS.txt`。
...
客户端按 Panel v1.2.2-rc1 使用 `/api/v1` Bearer Token API，包括注册、登录、bootstrap、节点、用户状态、5 秒流量上报、更新检查、修改密码与退出。登录 Token 使用 Android Keystore AES/GCM 加密保存；节点配置只交给本机 Core 生成器，不在 UI 中展示或提供复制入口。
